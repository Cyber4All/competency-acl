...
describe("Validate Valid Acls", () => {
	it(`Should return an array for ${competencyAcl.competencies.create}`, () => {
		expect(validateAcl(competencyAcl.competencies.create)).toEqual([
			competencyAcl.competencies.create,
		]);
	});
	it(`Should return an array for ${competencyAcl.actor.wildcard}`, () => {
		expect(new Set(validateAcl(competencyAcl.actor.wildcard))).toEqual(
			new Set([
	...
	it(`Should return an array for ${competencyAcl.competencies.getWildcard}`, () => {
		expect(
			new Set(validateAcl(competencyAcl.competencies.getWildcard))
		).toEqual(new Set(competencyGetWildcard));
	});
});
...
describe("Condense an Array of Acls", () => {
	...
	it("Should return a partially condensed array given a large array of acls", () => {
		expect(new Set(condenseAcl(aclArray.concat(additional)))).toEqual(
			new Set(correctArray.concat(additional))
		);
	});

	it("Should return the get wildcard from an array of get permissions", () => {
		expect(new Set(condenseAcl(competencyGetWildcard))).toEqual(
			new Set([competencyAcl.competencies.getWildcard])
		);
	});

	it("Should return a get wilcard plus the other permissions", () => {
		const newAcl = competencyGetWildcard.concat(additional);
		expect(new Set(condenseAcl(newAcl))).toEqual(
			new Set([competencyAcl.competencies.getWildcard].concat(additional))
		);
	});

	it("Should return competency:competencies:* given all competencies permissions", () => {
		expect(new Set(condenseAcl(competencyWildcard))).toEqual(
			new Set([competencyAcl.competencies.wildcard])
		);
	});
	...
	it("Should thrown an error for a poorly formatted acl", () => {
		expect(() => {
			return condenseAcl(["competency:helloThere"]);
		}).toThrowError();
	});
});
